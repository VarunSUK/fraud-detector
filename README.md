...
This is a fraud detection system built to actually run in production, not just to look good in a demo. It scores financial transactions in real time using a real trained model, turns that score into an actual credit risk decision, keeps an audit trail of every decision it makes, and gives a human reviewer a place to step in when the model isn't confident. Every piece of it, the model serving, the decisioning, the monitoring, and the deployment setup, is meant to hold up the way it would need to at a real company, not just on a resume.

## Why I Built This

I didn't want another portfolio project that only proves I can get a good score in a notebook. Anyone can train a model that hits 99% AUC on a clean dataset. What I actually wanted was something that works from end to end: a model that a real service actually calls, a score that turns into a real decision, and a system I could run, break, and fix the same way I'd have to at an actual job.

It started as a fairly ordinary project skeleton. A training script, a Go API, some Kubernetes config, and a README describing a system that was a lot more finished on paper than what was actually running underneath it. Closing that gap between what a project claims and what it actually does when you run it turned into most of the real work, and it's where almost everything below came from.

## What I Learned Building It

**A trained model isn't a product.** The inference service originally scored every transaction using a hardcoded mock, a handful of if and else thresholds pretending to be machine learning, while the real LightGBM and XGBoost training pipeline sat completely disconnected and was never actually called. Wiring the two together properly, with a real serving layer, health checks, and a graceful fallback when that service is unreachable, turned out to be a bigger and more interesting problem than training the model in the first place.

**A raw score is not a decision.** A number like 0.74 doesn't tell a credit risk analyst anything they can act on. Turning that into something like "decline this transaction, tighten this account's credit limit by 30 percent, and here's why" needed reason codes, an actual policy, and an audit trail someone could review later. That's where the real judgment lives, and it's a completely different skill from training a classifier.

**Compiling and running are not the same claim.** At one point the Go service had two HTTP routes registered on the same path and panicked the moment it started. The service in this repo had never actually been started successfully before I ran it myself for the first time. Tests that exercise the real router, not just individual functions on their own, are what caught it.

**Calibration matters more than which model you pick.** Swapping one gradient booster for another only changes a number on a slide. Realizing that neither one's raw output is a real probability, and that a policy saying "decline anything above 0.85" means nothing until the score is actually calibrated to that scale, changed how the whole scoring pipeline is built.

**Concurrency bugs hide behind averages.** A load test looked completely fine on the median response time. It was the tail latency that gave away a thundering herd bug, where every simultaneous request was independently rechecking a downstream service's health instead of sharing one result. It stayed invisible until something finally asked for real numbers instead of a single successful response.

**Security hardening has consequences that are easy to miss.** Turning on a hardened, non root, read only filesystem policy in Kubernetes broke nginx outright, since it wants to bind port 80 as root, and it would have quietly broken the Python services too, since /tmp isn't writable on a read only root by default. None of that shows up until you actually try to run the pods. It's easy to declare in YAML and never once check that it works.

The result is a system that actually functions from start to finish: real trained models behind a real serving layer, a credit risk decision engine that turns a score into an action, a human review queue, a live analytics dashboard, and a frontend that shows all of it happening as it runs. Not a project that looks finished on the surface while quietly not working underneath, which is exactly the trap this whole thing started in.
